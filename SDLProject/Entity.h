...
enum EntityType { PLATFORM, PLAYER, ENEMY, BACKGROUND, OBJECTS, TEXT, GIRL, BALL  };
enum AIType     { WALKER, GUARD, RUNNER            };
...
public:
    ...
    void update(float delta_time, Entity *player, Entity *enemies, int enemy_count, Entity *objects, int OBJECT_COUNT, Entity *squirrel, Entity *ball, Entity *line, Map *map);
    void render(ShaderProgram *program, float delta_time);
    void renderbg(ShaderProgram *program);
    void display_text(ShaderProgram *program);
    void activate_ai(Entity *player);
    void ai_walker();
    void ai_guard(Entity *player);
    void ai_runner(Entity *player);
    ...
    void const set_string(std::string new_line)             { line         = new_line;             };
};
