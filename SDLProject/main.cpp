...
void initialise()
{
    ...
    display_window = SDL_CreateWindow("Fetch!",
...
void update()
{
    ...
    if (current_scene == levelA && (key_state[SDL_SCANCODE_RETURN])) {
        switch_to_scene(levelB);
        effects->start(FADEIN, 1.0f);
    }
    if (current_scene == levelB && current_scene->state.player->position.x > 10){
        switch_to_scene(levelC);
        effects->start(FADEIN, 0.5f);
    }
    
    if (current_scene == levelC && (key_state[SDL_SCANCODE_A])) {
        switch_to_scene(levelD);
        effects->start(FADEIN, 0.5f);
    }
    if (current_scene == levelD && (key_state[SDL_SCANCODE_B])) {
        switch_to_scene(levelE);
        effects->start(FADEIN, 0.5f);
    }
    if (current_scene == levelE && (key_state[SDL_SCANCODE_RETURN])) {
        switch_to_scene(levelF);
        effects->start(FADEIN, 0.5f);
    }
    if (current_scene == levelF && current_scene->state.player->check_collision(current_scene->state.enemies)) {
        switch_to_scene(levelF);
        effects->start(FADEIN, 0.5f);
    }
...
//    delta_time += accumulator;
    ...
    glUseProgram(program.programID);
    current_scene->render(&program, delta_time);
    effects->render();
