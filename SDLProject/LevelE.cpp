...
void LevelE::initialise()
{
    ...
    this->state.map = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVELE_DATA, map_texture_id, 1.0f, 4, 1);
    ...
    font4_texture_id = Utility::load_texture(FONT);
}
...
//    this->state.BG->model_matrix = glm::scale(this->state.BG->model_matrix, glm::vec3(0.0f, 0.0f, 0.0f));
}
void LevelE::render(ShaderProgram *program, float delta_time)
{
    this->state.BG->renderbg(program);
    Utility::draw_text(program, font4_texture_id, "beach day!", 0.5f, 0.00000000001f, glm::vec3(2.0f, -6.0f, 0.0f));
}
